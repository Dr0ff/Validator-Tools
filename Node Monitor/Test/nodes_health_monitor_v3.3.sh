...
declare -i MAX_RETRIES=5         # Максимальное количество попыток получить данные (1 = одна попытка без повторов)
declare -i RETRY_DELAY_SECONDS=10 # Задержка между попытками в секундах
