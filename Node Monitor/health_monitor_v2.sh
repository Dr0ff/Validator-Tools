...
# Файл для логов отладки.
# Он будет находиться в той же директории, что и сам скрипт.
DEBUG_LOG_FILE="${CURRENT_SCRIPT_DIR}/health_debug.log" # Или любое другое имя файла

# Функция для записи отладочных сообщений в отдельный файл
log_debug() {
...
if [[ "$1" == "--debug" ]]; then
    ...
    # Теперь это сообщение пойдет в DEBUG_LOG_FILE, а не в stdout
    log_debug "  * Глобальный режим отладки включен. * "
    log_debug "----------------------------------------------------------------------------"
    log_debug " Запуск скрипта мониторинга: health_monitor.sh. Время: $(date -u -R)"
    log_debug "----------------------------------------------------------------------------"
fi
...
log_debug "ОТПРАВКА_СУТОЧНОГО_ОТЧЕТА_ВЫЗВАНА_ДЛЯ: ${node_name^^}"
            send_telegram "📊 Ежедневный отчёт для ${node_name^^}:%0AЗа прошедшие сутки пропущено: ${MISSED_FOR_24H} блоков.%0AТекущий счетчик: ${CURRENT_MISSED_BLOCKS}." "REPORT" "INFO"
        else
...
log_debug "----------------------------------------------------------------------------"
log_debug "Завершение скрипта мониторинга: health_monitor_v2.sh. Время: $(date -u -R)"
log_debug "----------------------------------------------------------------------------"
log_debug ""
